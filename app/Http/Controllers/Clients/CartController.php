...
class CartController extends Controller
{
    public function addToCart(Request $request)
    {
        ...
        if (Auth::check()) {
            ...
            //if not  login then save to seesion
    ...
    public function loadMiniCart()
    {
        ...
        return response()->json([
            ...
            'html' => view('clients.components.includes.mini_cart', [
                'cart' => $cartItems // Đổi tên phím thành 'cart' để khớp với View
            ])->render()
        ]);
    }
    ...
    public function removeFormMiniCart(Request $request)
    {
        ...
        if (Auth::check()) {
            $cartData = CartItem::with('product')->where('user_id', Auth::id())->get();
        } else {
