$(document).ready(function () {
    ...
    $(document).on("click", ".add-to-cart-btn", function (e) {
        ...
        $.ajax({
            ...
            error: function (xhr) {
                alert("Có lỗi xảy ra với ajax addtocartdetail!");
            },
        });
    });
    ...
    $(".remove-from-cart").on("click", function (e) {
        ...
        $.ajax({
            ...
            success: function (response) {
                console.log("Dữ liệu tiền mới từ server:", response);
                row.remove();
                $(".cart-total").text(response.total);
                $(".cart-grand-total").text(response.grandTotal);
    ...
    var totalPriceNumber = parseFloat(
        totalPriceText.replace(/\./g, "").replace(" d", ""),
    );
                ...
                createOrder: function (data, actions) {
                    return actions.order.create({
                        purchase_units: [
                            {
                                amount: {
                                    value: (totalPriceNumber / 2500).toFixed(2),
                                },
                            },
                        ],
                    });
    ...
    $(document).on("click", ".wishlist-product-remove", function (e) {
        ...
        $.ajax({
            ...
            error: function (xhr) {
                alert("Có lỗi xảy ra với ajax remove wishlist!");
            },
        });
    });
    /***********************************************
